...
template<class datatype>
double runbench(int total_blocks, datatype *cd, long size, bool spreadsheet, bool readonly, int stepwidth, int index_clamping){
    ...
    // collect power data
    int iters = int((double)secs * 1000 / kernel_time);
    printf("Adjust %d iterations to achieve time duration %d.\n", iters, secs);

	initializeEvents(&start, &stop);
	cudaEvent_t mystart, mystop;
	checkCudaErrors(cudaEventCreate(&mystart));
	checkCudaErrors(cudaEventCreate(&mystop));
	float  total_20_time = 0;
	float msecTotal = 0.0f;
    for (int i = 1 ; i <= iters; i++){
		checkCudaErrors(cudaEventRecord(mystart, NULL));
	    benchmark_func<datatype>(dimGrid, dimBlock, cd, readonly, BLOCK_SIZE, stepwidth, index_clamping);
		cudaThreadSynchronize();
		checkCudaErrors(cudaEventRecord(mystop, NULL));
		checkCudaErrors(cudaEventSynchronize(mystop));
		msecTotal = 0 ;
		checkCudaErrors(cudaEventElapsedTime(&msecTotal, mystart, mystop));
		total_20_time+=msecTotal;
		if(i%20==0){
			    printf(" last 20 iterats, average time is %f msec\n",(double)total_20_time/20);
                total_20_time=0;
		}
	}
...
template<class datatype>
double cachebenchGPU(double *c, long size, bool excel, bool readonly, int stepwidth, int index_clamping){
	// Construct grid size
	cudaDeviceProp deviceProp;
	int current_device;
	CUDA_SAFE_CALL( cudaGetDevice(&current_device) );
    ...
    // only one case
	peak_bw = max( peak_bw, runbench<datatype>(TOTAL_BLOCKS, cd, size, excel, readonly,  stepwidth, index_clamping) );
...
extern "C" void cachebenchGPU(double *c, long size, bool excel, bool readonly, int benchtype, int stepwidth, int index_clamping, int time ){
#ifdef L2_ONLY
    ...
    double peak_bw_int = 0.0;
    switch(benchtype){
        default:
        case 0:
            peak_bw_int = cachebenchGPU<int>(c, size, excel, readonly, stepwidth, index_clamping);
            break;
        case 1:
	        peak_bw_int = cachebenchGPU<int2>(c, size, excel, readonly, stepwidth, index_clamping);
            break;
        case 2:
	        peak_bw_int = cachebenchGPU<int4>(c, size, excel, readonly, stepwidth, index_clamping);
            break;
    }
