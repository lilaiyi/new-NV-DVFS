	...
	do
	{
		...
		if (k == 7){
			for (int i = -20; i < nIter; i++){
				...
				checkCudaErrors(cudaEventRecord(startE, NULL));

				bfs << < grid, threads, 0 >> >(d_graph_nodes, d_graph_edges, d_graph_mask, d_updating_graph_mask, d_graph_visited, d_cost, no_of_nodes);

				cudaDeviceSynchronize();
				checkCudaErrors(cudaEventRecord(stopE, NULL));

				// Wait for the stop event to complete
				checkCudaErrors(cudaEventSynchronize(stopE));
				float msecTotal = 0.0f;
				checkCudaErrors(cudaEventElapsedTime(&msecTotal, startE, stopE));
		...
		reset << < grid, threads, 0 >> >(d_graph_mask, d_updating_graph_mask, d_graph_visited, d_over, no_of_nodes);
		cudaThreadSynchronize();
