...
import configparser
import json
root_double = '/home/lyli/NV-DVFS-Benchmark-master/tools/csvs/test/'
        ...
        if j==i: 
             ...
             for argNo_1, arg_1 in enumerate(args):
                for argNo_2, arg_2 in enumerate(args_2):
                    ...
                    Performance_csv_file_app1 = [file_name for file_name in files if file_name.endswith("%sPerformance.csv"%appname1)]
                    ...
                    target_df = target_df.merge(df3[["coreF", "power","var"]], on=[ "coreF"], how="left")
                    ...
                    POWER_csv_file_app1 = [file_name for file_name in files_app1 if file_name.endswith("POWER.csv")]
                    POWER_csv_file_app2 = [file_name for file_name in files_app2 if file_name.endswith("POWER.csv")]
                    Performance_csv_file_app1 = [file_name for file_name in files_app1 if file_name.endswith("%sPerformance.csv"%appname1)]
                    Performance_csv_file_app2 = [file_name for file_name in files_app2 if file_name.endswith("%sPerformance.csv"%appname2)]
             ...
             for argNo_1, arg_1 in enumerate(args):
                for argNo_2, arg_2 in enumerate(args_2):
                    ...
                    dcgm_csv_file = [file_name for file_name in files if file_name.endswith("dcgm.csv")]
                    Performance_csv_file_app1 = [file_name for file_name in files if file_name.endswith("%sPerformance.csv"%appname1)]
                    Performance_csv_file_app2 = [file_name for file_name in files if file_name.endswith("%sPerformance.csv"%appname2)]
                    ...
                    merged_df = pd.concat([merged_df, target_df], ignore_index=True)
merged_df.to_csv("merged_file_with_single_dcgm.csv", index=False)
