...
import configparser
import json
from utils.profiler2 import PowerProfiler 
from utils.profiler2 import NvProfiler
from utils.profiler2 import DCGMProfiler
...
if __name__ == '__main__':
    ...
    parser.add_argument('--percentage_kernel_1', type=str, help='CUDA_MPS_ACTIVE_THREAD_PERCENTAGE', default='90')
    parser.add_argument('--percentage_kernel_2', type=str, help='CUDA_MPS_ACTIVE_THREAD_PERCENTAGE', default='10')
                ...
                if j==i: 
                    ...
                    for argNo_1, arg_1 in enumerate(args):
                        for argNo_2, arg_2 in enumerate(args_2):
                            ...
                            logging_dir =  'logs/microbenchmark/%s-%s-%d_%s-%d_percentage%s:%s-no%d' % (opt.benchmark_setting, app1, argNo_1,app2,argNo_2,opt.percentage_kernel_1,opt.percentage_kernel_2,count)
                            try:
                            ...
                            if bench_args['dcgm'] is not None:
                                dcgm_profiler.end()
                            dcgmlog_size = os.path.getsize(dcgmlog)
                            if dcgmlog_size == 0:
                                print("dcgm error!count=%d",count)
                                exit()
                            if bench_args['nvprof'] is not None:
